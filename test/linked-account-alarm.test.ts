...
test('ensure resources exist to create a linked account alarm with existing sns topic', () => {
  ...
  const config: LinkedAccountAlarmProps = {
    ...
    accountConfiguration: [
      { accountId: '444455556666', alarmDescription: 'Consolidated billing alarm for all AWS service charge estimates (Account: 444455556666)', thresholdAmount: 50 },
    ],
  };
...
test('ensure resources exist to create a linked account alarm with existing sns topic with specific aws service', () => {
  ...
  const config: LinkedAccountAlarmProps = {
    secretName: 'test/billing/topicArn',
    accountConfiguration: [
      { accountId: '444455556666', alarmDescription: 'Billing Alarm for AWS DynamoDB charge estimates only (Account: 444455556666)', thresholdAmount: 50, awsService: 'AmazonDynamoDB' },
    ],
  };
...
test('ensure resources exist to create a billing alarm for multiple linked accounts', () => {
  ...
  const config: LinkedAccountAlarmProps = {
    ...
    accountConfiguration: [
      { accountId: '444455556666', alarmDescription: 'Consolidated billing alarm for all AWS service charge estimates (Account: 444455556666)', thresholdAmount: 50 },
      { accountId: '123456789000', alarmDescription: 'Consolidated billing alarm for all AWS service charge estimates (Account: 123456789000)', thresholdAmount: 120 },
    ],
  };
